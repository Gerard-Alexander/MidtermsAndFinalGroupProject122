...
public class EquivalentNumbers implements NumberConverter {
    ...
    public static String decimalToHexadecimal(double decimal) {
        int wholeNumber = (int) Math.abs(decimal);
        double fractionalPart = Math.abs(decimal) - wholeNumber;
        ...
        String hexadecimalString = (decimal % 1 == 0) ? wholeNumberHexadecimal :
                wholeNumberHexadecimal + fractionalHexadecimal.toString().toUpperCase(); // Combine whole and fractional parts

        System.out.println(hexadecimalString); // Print the hexadecimal string

        return (decimal < 0) ? "-" + hexadecimalString : hexadecimalString; // Return the hexadecimal string
    }
